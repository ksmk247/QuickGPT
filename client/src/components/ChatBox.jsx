  ...
  useEffect(() => {
    if(containerRef.current){
      containerRef.current.scrollTo({
        top: containerRef.current.scrollHeigth,
        behavior: 'smooth',
      })
    } 
  },[messages])
  ...
  return (
    <div className='flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40'>
      ...
      <div ref={containerRef}  className='flex-1 mb-5 overflow-y-scroll'>
        ...
        {mode === 'image' && (
          <label className='inline-flex items-center gap-2 mb-3 text-sm mx-auto'>
            <p className='text-xs'> Publish generation Image to Community.</p>
      ...
      <form onSubmit={onSubmit} className='bg-primary/20 dark:bg-[#583C79]/30 border border-primary dark:border-[#80609f]/30 rounded-full w-full max-w-2xl p-3 pl-4 mx-auto flex gap-4 items-center'>
        ...
        <input onChange={(e) => setPrompt(e.target.value)} value={prompt} type="text" placeholder='type your prompt here..' className='flex-1 w-fu; text-sm outline-none' required />
